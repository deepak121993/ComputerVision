...
class ResNet:
    ...
    def residual_module(data,K,stride,chanDim,red=False,reg=0.0001,bnEps=2e-5,bnMom=0.9):
        ...
        conv2 =  Conv2D(int(K*0.25),(3,3),use_bias=False,kernel_regularizer=l2(reg))(act2)
    ...
    def build(width,height,depth,classes,stages,filters,reg=0.0001,bnEps=2e-5,bnMom=0.9,dataset="cifar"):
        ...
        for i in range(0,len(stages)):
            ...
            x= ResNet.residual_module(x,filters[i+1],stride,chanDim,red=True,reg,bnEps,bnMom)

            for j in range(0,stages[i]-1):
                x = ResNet.residual_module(x,filters[i+1],(1,1),chanDim,red=True,reg,bnEps,bnMom)
